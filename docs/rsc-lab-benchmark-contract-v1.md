...
This contract preserves the July public-demo experiment and must not be used as
the current ShakaPerf claim. Its runner was Ruby/Selenium
(`scripts/perf/compare_dashboard_routes.rb`), not the `shaka-perf` CLI. Current
native-product evidence is in the [August 12 ShakaPerf artifact](./performance-artifacts/native-product-rsc-shakaperf-2026-08-12/README.md).

## Supported historical claim

This versioned contract governed the public buyer-page benchmark. Its historical evidence is the serialized, same-host Ruby/Selenium A/B run of these deterministic route pairs:
...
1. **Historical A/B:** the stable-host, media-bearing Ruby/Selenium result captured July 10, 2026 UTC: two independent batches of eight alternating cycles per pair, with two warmups before each measured run. The immutable summary, four manifests, and 64 raw measurements live in [`performance-artifacts/deployed-stable-media-public-buyer-pages-2026-07-10`](./performance-artifacts/deployed-stable-media-public-buyer-pages-2026-07-10/README.md). This is the only evidence governed by v1, not the repository's current headline.
...
Final-release validation was deliberately deferred; it is not satisfied by the historical run.
