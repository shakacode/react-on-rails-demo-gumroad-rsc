...
*Draft for review. Publication venue is not yet selected, and the real product-page result below is pending.*

This experiment is run by ShakaCode, which maintains React on Rails and sells React on Rails Pro. [The repository is a public fork of `antiwork/gumroad` and is not Gumroad's canonical repository](../../README.md).

Gumroad's company account published "How Gumroad Migrated from React on Rails to Inertia.js," describing a sensible response to real engineering friction. Its old frontend combined Rails-rendered layouts, React mounts, internal API calls, client-side sub-routing, and separate server and browser builds. Navigation discarded stable page chrome. Data already available to Rails took a detour through frontend-only endpoints. Forms and routing crossed framework boundaries. Each concern was manageable, but together they raised the cost of ordinary product work.
...
That is a good trade for a creator dashboard. It is an authenticated application surface, used repeatedly, where persistent navigation and straightforward Rails ownership matter more than search metadata or a fully rendered first document. Nothing in our experiment changes that conclusion. Our position starts from the same premise: Inertia is excellent for many Rails pages, and ignoring its strengths would make a comparison ideological rather than practical.

## The backend thesis continues

The most useful sentence in "How Gumroad Migrated from React on Rails to Inertia.js" is also the bridge to our experiment:
...
The distinction matters because dashboards and public buyer pages have different jobs. A dashboard benefits from warm caches, repeat visits, and persistent application state. A public product or marketplace page often meets a new visitor on an uncertain network. The initial document, metadata, and amount of client work can affect discovery and conversion. Our lab therefore measures a different class of page: logged-out Product and Discover surfaces, not the creator workflow that motivated most of Gumroad's migration. [The benchmark contract keeps that public-page boundary explicit](../rsc-lab-benchmark-contract-v1.md#status-and-supported-claim).
...
[Inertia's own SSR path](https://inertiajs.com/server-side-rendering) introduces a separate Node server and a separately built SSR bundle, conventionally `ssr.js`. If an Inertia application adds SSR after removing it, a second runtime and server build return. React Server Components through React on Rails Pro also require a Node renderer and distinct server and client assets. [The current lab documents those actual moving parts](../current-status.md#react-192--pro-17-status).

So the useful comparison is not a server-rendered RSC route against no extra process. It is one server-rendering design against another, asking what the additional process buys. In this lab, it buys streamed rendered output and an isolated route bundle whose JavaScript transfer is about half the control's. How much of that difference is intrinsic to the architecture is bounded by the causal limit below. The design also adds deployment, observability, and failure modes that a client-rendered Inertia page does not have.

## The page data has two representations

On an initial Inertia response, Rails serializes the page props as JSON in the document's `data-page` attribute. The browser then uses those props to construct the React DOM. The payload and the rendered interface are separate representations of the same page data. This is easy to verify without a benchmark: open an Inertia page, then inspect the root element or the page-data script tag for `data-page`.

In the deployed lab, the Product control carried [`15,040 B`](../performance-artifacts/deployed-stable-media-public-buyer-pages-2026-07-10/summary.json) in that attribute, while Discover carried [`33,966 B`](../performance-artifacts/deployed-stable-media-public-buyer-pages-2026-07-10/summary.json). The RSC candidates had no Inertia `data-page` payload. Instead, the initial response carried rendered HTML plus the RSC stream used by the React runtime. [The repository documents that delivery shape](../current-status.md#react-192--pro-17-status). This is an architectural difference, but the byte totals below are properties of these particular pages and fixtures, not universal constants. The `data-page` figures are uncompressed sizes of the serialized page data, while the HTML column below is the compressed encoded body, so they are not directly comparable on the wire.

## What the deployed lab measured

The canonical run was captured on [July 10, 2026 at application commit `cc61125b02ec0282ec455c044240e97b6a33b741`](../performance-artifacts/deployed-stable-media-public-buyer-pages-2026-07-10/summary.json). It used [two independent batches of eight alternating cycles per route pair, two server warmup requests before each measured run, and sixteen samples per route](../performance-artifacts/deployed-stable-media-public-buyer-pages-2026-07-10/summary.json). The table reports Inertia control to RSC candidate:

| Surface | Median navigation | Median response end | Median LCP | Encoded HTML body | JavaScript transfer | Inertia `data-page` (uncompressed) |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
...
The strongest result is full-navigation duration. Product LCP improved modestly. Discover LCP moved in the same direction but was noisier: [two of sixteen paired cycles regressed, and its batch medians differed materially](../performance-artifacts/deployed-stable-media-public-buyer-pages-2026-07-10/summary.json). Response end is not an RSC win: a tie on Product and inconclusive on Discover, [where the batch medians disagree in direction](../performance-artifacts/deployed-stable-media-public-buyer-pages-2026-07-10/summary.json). RSC also sent substantially more HTML, even as JavaScript transfer fell by about half.
...
[Both arms are purpose-built benchmark routes over the same curated fixture](../public-page-fixture-sampling.md). [The Inertia control is not a port of Gumroad's production components](../public-product-rsc-demo.md#production-shaped-fixtures). The Discover fixture is synthetic; the Product fixture is a curated rewrite attributed to one public product. Product loads [five small SVGs totaling about `11 KB`](../current-status.md#live-gumroad-parity-audit), while the sampled live Product page transferred roughly [`2.3-3.2 MB`](../current-status.md#live-gumroad-parity-audit) of mixed media. Discover uses [eight unique SVGs totaling about `17 KB`](../current-status.md#live-gumroad-parity-audit), while sampled live Discover runs transferred roughly [`11-16 MB` across about `100` image requests](../current-status.md#live-gumroad-parity-audit). Those are orders of magnitude apart.

The run used [unthrottled desktop headless Chrome on a public network](../performance-artifacts/deployed-stable-media-public-buyer-pages-2026-07-10/summary.json). It is not a mobile result and not field data. A transient [`503`](../performance-artifacts/deployed-stable-media-public-buyer-pages-2026-07-10/summary.json) occurred while the deployment woke before the media gate passed. The harness also opened the shared host root before each target, warming the connection and common resources equally but making the visit less than fully cold. [The capture predates the contract that now governs reruns, and its media-gate output and exact arm-to-arm media inventory comparison were not preserved](../rsc-lab-benchmark-contract-v1.md#historical-capture-exception). These limitations narrow the claim. They do not erase the observed route difference.

## Real product-page run

<!-- PENDING: real-product-page run, numbers land separately -->

A pending follow-up run will use a real product-page implementation rather than the matched synthetic pair. It matters more because real media, chrome, application code, caching, and third parties determine whether the laboratory advantage survives those conditions. Its results will be reported separately rather than folded back into the canonical lab artifact.

## What adopting this route costs

The candidate is not a configuration switch. It requires [React 19](../../package.json), [a Shakapacker and Rspack build path](https://github.com/shakacode/react-on-rails-demo-gumroad-rsc/blob/63ee9512b56ca4a56fe3a0bece27aeb0d1a2f7cd/README.md?plain=1#L76-L83) instead of [Gumroad's Vite path](https://github.com/shakacode/react-on-rails-demo-gumroad-rsc/blob/63ee9512b56ca4a56fe3a0bece27aeb0d1a2f7cd/README.md?plain=1#L74), a separately operated Node renderer, and a commercial React on Rails Pro license. Rspack is supporting build infrastructure, not the explanation for the runtime result. The deployment guide [names the license and renderer environment variables](../control-plane-deployment.md).

That means adoption should be selective. A team has to price the React upgrade, asset-pipeline change, renderer capacity and monitoring, fallback behavior, and license against the value of the public surface. The dashboard case may still favor Inertia's simpler client-rendered model. The public Product or Discover case may justify the extra machinery if the measured reductions in navigation duration and route JavaScript transfer persist with matched media and third-party conditions.

## A reproducible choice, not a migration demand

The [public experiment repository](https://github.com/shakacode/react-on-rails-demo-gumroad-rsc), the [benchmark contract](../rsc-lab-benchmark-contract-v1.md), and the [canonical artifact](../performance-artifacts/deployed-stable-media-public-buyer-pages-2026-07-10/summary.json) expose the method, boundaries, and caveats with the numbers. The Inertia routes remain the control, a practical architecture for many pages, and a fallback while the RSC path is tested.
...
Gumroad is a trademark of Gumroad, Inc. This experiment is not affiliated with or endorsed by Gumroad, Inc.
