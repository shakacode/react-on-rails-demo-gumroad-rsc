...
[Both arms are purpose-built benchmark routes over the same fixture, and the Inertia control is not a port of Gumroad's production components](../../README.md#current-public-buyer-page-implementation). The fixtures are synthetic. Product loads [five small SVGs totaling about `11 KB`](../current-status.md#live-gumroad-parity-audit), while the sampled live Product page transferred roughly [`2.3-3.2 MB`](../current-status.md#live-gumroad-parity-audit) of mixed media. Discover uses [eight unique SVGs totaling about `17 KB`](../current-status.md#live-gumroad-parity-audit), while sampled live Discover runs transferred roughly [`11-16 MB` across about `100` image requests](../current-status.md#live-gumroad-parity-audit). Those are orders of magnitude apart.
...
The candidate is not a configuration switch. It requires [React 19](../../package.json), [a Shakapacker and Rspack build path instead of Gumroad's Vite path](../../README.md#july-2026-react-on-rails-pro-17--react-192-audit), a separately operated Node renderer, and a commercial React on Rails Pro license. Rspack is supporting build infrastructure, not the explanation for the runtime result. The deployment guide [names the license and renderer environment variables](../control-plane-deployment.md).
