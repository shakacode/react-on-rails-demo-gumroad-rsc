...
The dashboard comparison requires sign-in. Review and staging release phases set
`ALLOW_DEMO_SEED=true` from `.controlplane/controlplane.yml`, so use:
...
internal-admin access from `seller@gumroad.com`. The flag is not persisted in the
shared Control Plane app template because that template can be applied to
production and production promotion checks staging/prod env parity. Local
development keeps the normal internal-admin/2FA behavior documented in
`docs/users.md`.
