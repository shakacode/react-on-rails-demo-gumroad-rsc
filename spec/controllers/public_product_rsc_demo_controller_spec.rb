...
describe PublicProductRscDemoController, type: :controller, inertia: true do
  ...
  describe "GET performance_demo" do
    ...
    it "renders a logged-out public performance lab for product and Discover route pairs" do
      ...
      expect(response.body).to include("Native product pages: actual ShakaPerf CLI A/B")
      ...
      expect(response.body).to include("Fairness, stated first")
      expect(response.body).to include("Google Tag Manager/Analytics and the Facebook SDK")
  ...
  describe "GET executive_summary" do
    it "renders a short logged-out decision brief before the detailed evidence lab" do
      ...
      expect(response.body).to include("Known gaps, stated first")
      expect(response.body).to include("Google Tag Manager/Analytics and the Facebook SDK")
      expect(response.body).to include("not an RSC-alone effect")
      expect(response.body).to include("-48.8%")
      expect(response.body).to include("-42.6%")
      ...
      expect(response.body).to include("The full route-pair cost includes the matched shared surface")
      expect(response.body).to include("same presenter and fixture wiring, the shared styles")
      expect(response.body).to include("matched 12-line Inertia wrapper")
      expect(response.body).to include("The incremental RSC second-arm cost is small once that matched shared surface exists")
      expect(response.body).to include("Unrelated routes can remain on Inertia")
      expect(response.body).to include("this Product migration does not require converting them")
      ...
      expect(response.body).to include("ReactOnRailsPro::Stream")
      expect(response.body).to include("LiveStreamingResponseHeaders")
      expect(response.body).to include("LiveActiveRecordConnectionCleanup")
      expect(response.body).to include("before_action :prepare_live_streaming_response")
      expect(response.body).to include("prepend_around_action :clear_live_active_record_connections")
      expect(response.body).to include("DashboardComparisonTiming")
      expect(response.body).to include("callback provide demo observability")
      expect(response.body).to include("neither is a universal RSC requirement")
      ...
      expect(response.body).to include(PublicProductRscDemoPresenter::REACT_ON_RAILS_GITHUB_URL)
      ...
      expect(response.body).to include("full inventory and cleanup of every Pro-backed surface")
      expect(response.body).to include("PublicProductRscDemoController")
      expect(response.body).to include("DashboardRscDemoController")
      expect(response.body).to include("retained public")
      expect(response.body).to include("dashboard RSC routes")
      expect(response.body).to include("all client and server registrations")
      expect(response.body).to include("RSC bundle/build")
      expect(response.body).to include("Pro package dependencies")
