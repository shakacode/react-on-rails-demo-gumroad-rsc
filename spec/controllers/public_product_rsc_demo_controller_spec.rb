...
describe PublicProductRscDemoController, type: :controller, inertia: true do
  ...
  describe "GET performance_demo" do
    it "renders a logged-out public performance lab for product and Discover route pairs" do
      ...
      expect(response.body).to include("Current branch ShakaPerf A/B result")
      expect(response.body).to include("hosted review-app A/B summary")
      expect(response.body).to include("Lighthouse URL-pair summary")
      expect(response.body).to include("Hosted review-app rerun")
      expect(response.body).to include("602.75ms to 502.20ms")
      expect(response.body).to include("0.56 to 0.99")
      expect(response.body).to include("Compare deployed demo")
      expect(response.body).to include("deployed-demo PageSpeed")
      ...
      expect(response.body).to include("Product detail mobile current-app PageSpeed")
      expect(response.body).to include("Product detail mobile live PageSpeed")
