...
The demo is on the React on Rails Pro 17 RC line with React 19.2.7, React on Rails Pro gem `17.0.0.rc.12`, React on Rails Pro npm `17.0.0-rc.12`, and `react-on-rails-rsc` `19.2.1`. The public RSC routes now opt into Pro stream observability so `Server-Timing` can include streamed shell and Node renderer prepare attribution when the renderer returns it.
