...
| --- | --- |
| `+review-app-deploy` | Resolves the exact PR head, builds its image, creates the review app if needed, deploys, waits for health, and comments with the review URL. |
...
The automatic pull-request run can intentionally stop after validation when the
review app does not exist yet. That no-op is not deployment proof. An authorized
`+review-app-deploy` comment creates the first app; completion requires the
workflow to report exact-head build, deploy, and health success.
...
release tag your wrappers are pinned to: a `CPFLOW_VERSION=5.2.0` runtime
override goes with a wrapper pinned to `uses: ...@v5.2.0`. For later releases,
use the same exact version on both sides. This repository rejects stable
releases older than `v5.2.0` because they predate health-aware completion.
